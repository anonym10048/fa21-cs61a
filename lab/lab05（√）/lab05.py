...
def distance(city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    return sqrt(abs((get_lat(city_a) - get_lat(city_b)) ** 2 + (get_lon(city_a) - get_lon(city_b)) ** 2))
...
def closer_city(lat, lon, city_a, city_b):
    ...
    dist_a, dist_b = distance(city_c, city_a), distance(city_c, city_b)
...
def berry_finder(t):
    ...
    if label(t) == 'berry':
        return True
    for b in branches(t):
        return True if True in [berry_finder(b) for b in branches(t)] else False
