    ...
    while num_rolls >= 1:
        ...
        if num == 1:
            while num_rolls > 1:
                ...
                dice()
            return 1
        count, num_rolls = count + num, num_rolls - 1
    return count
    # END PROBLEM 1
...
def bacon_strategy(scores, opponent_score, cutoff=9, num_rolls=5):
...
def extra_turn_strategy(score, opponent_score, cutoff=12, num_rolls=5):
    """This strategy rolls 0 dice when it triggers an extra turn. It also
    rolls 0 dice if it gives at least CUTOFF points and does not give an extra turn.
    Otherwise, it rolls NUM_ROLLS.
    """
    # BEGIN PROBLEM 11
    if extra_turn(free_bacon(opponent_score) + score, opponent_score) or bacon_strategy(score, opponent_score) == 0:
        num_rolls = 0
    return num_rolls
    # END PROBLEM 11
...
def final_strategy(score, opponent_score, cutoff=12, num_rolls=5):
    ...
    """
    # BEGIN PROBLEM 12
    if 1 < score - opponent_score <= 3 or swine_align(score + 1, opponent_score):  # 针对swine align和pig pass的策略
        num_rolls = 10
    elif extra_turn_strategy(score, opponent_score) == 0:         # extra turn
        num_rolls = 0
    elif score - opponent_score > 20:                   # 为了能尽快拉开差距
        num_rolls = 5
    return num_rolls


print(final_strategy(1, 0))
