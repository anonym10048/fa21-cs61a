...
def choose(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    b = [paragraphs[i] for i in range(len(paragraphs)) if select(paragraphs[i])]
    if k >= len(b):
        return ''
    else:
        return b[k]
...
def about(topic):
    ...
    'Nice pup.'
    """
    from utils import lower, split, remove_punctuation
    assert all([lower(x) == x for x in topic]), 'topics should be lowercase.'
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"

    def func1(para):
        paragraph = split(remove_punctuation(lower(para)))
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    if typed_word in valid_words:
        return typed_word
    diff_list = [diff_function(typed_word, valid_word, limit) for valid_word in valid_words]
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    for i in range(len(prompt)):
        if i >= len(sofar) or prompt[i] != sofar[i]:
            break
        else:
            count += 1
    dict = {'id':user_id, 'progress':count / len(prompt)}
    upload(dict)
...
def time_per_word(words, times_per_player):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    time_stamp = [[i[j + 1] - i[j] for j in range(len(i) - 1)]for i in times_per_player]
    return match(words, time_stamp)
    # END PROBLEM 9
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    list = [[] for k in player_indices]
    for i in player_indices:
        for j in word_indices:
            if get_times(match)[i][j] == min([x[j] for x in get_times(match)]):
                if get_words(match)[j] in sum(list, []):
                    continue
                list[i].append(get_words(match)[j])
